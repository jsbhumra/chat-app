...
function RecMessTop({ details}) {
  return (
    <div className='flex justify-start'>
        <div className={`relative rounded-lg rounded-tl-none max-w-[60%] px-3 py-1 ml-10 mt-2 w-auto outline-1 outline-dashed -outline-offset-2`} style={{ backgroundColor: details.textColor }}>
            <div className='underline text-sm font-semibold'>{details.username}:</div>
            <div>{details.text}</div>
            <div className={`absolute w-0 h-0 border-t-[15px] border-l-[15px] border-l-transparent -left-[9px] top-0`} style={{ borderTopColor: details.textColor }}></div>
